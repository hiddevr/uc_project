...
class PPricing:
    ...
    def _ga_function(self, instance, solution, solution_idx):
        delta_T = solution
        delta_V = (solution * self.curr_fractions).astype(int)
        if sum(delta_V) != self.available_scooters:
            fitness = -99999999999
        else:
            fitness = self._objective_function(delta_T, delta_V)
        return fitness
    ...
    def run_algo(self):
        ...
        for j in range(24):
            ...
            self.t = j
            self._update_areas()
            self.curr_fractions = 1 + self.transition_matrix.loc[self.transition_matrix['End Hour'] == self.t][
                'Average Fraction'].copy().values

            ga_instance = pygad.GA(num_generations=10000,
                                   num_parents_mating=2,
                                   fitness_func=self._ga_function,
                                   sol_per_pop=100,
                                   num_genes=self.total_areas,
                                   gene_type=int,
                                   gene_space={'low': 0, 'high': min(200, self.available_scooters)},
